...
async def quiz_handler(event):
    ...
    if msg.poll and msg.poll.poll.quiz:
        ...
        # FIX: Newer Telethon versions require `hash` in types.Poll.__init__().
        # Use hash=0 for a freshly created poll (Telegram assigns the real hash on creation).
        poll = types.Poll(
            id=int(time.time()),
            hash=0,
            question=question,
        ...
        try:
            ...
            # FIX: Same hash=0 fix for the closed poll variant.
