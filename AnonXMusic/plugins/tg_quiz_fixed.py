...
def should_send_warning(user_id):
    """Check if we should send warning (rate limiting — max once per 30 s)"""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    try:
...
def remove_emojis_preserve_entities(twe: types.TextWithEntities) -> types.TextWithEntities:
    ...
    for entity in entities:
        ...
        removed_before = sum(m.end() - m.start() for m in emoji_matches if m.end() <= old_start)
        removed_within = sum(m.end() - m.start() for m in emoji_matches if old_start < m.end() <= old_end)
        new_start = old_start - removed_before
        new_length = entity.length - removed_within
        if new_length > 0:
...
async def again_handler(event):
    ...
    try:
        ...
        if not joined_all:
            missing_channels = list(REQUIRED_CHANNELS)
            await send_channel_warning(event, missing_channels)
            return
...
async def quiz_handler(event):
    ...
    if msg.poll and msg.poll.poll.quiz:
        ...
        # Build answers with fresh sequential raw-byte option identifiers.
        # QuizBot uses ASCII bytes (b'0', b'1', …) which Telegram rejects when
        # creating a brand-new poll.  We generate b'\x00', b'\x01', … instead
        # and remap the correct answer accordingly.
        option_map = {}   # QuizBot option bytes  →  our new option bytes
        answers = []
        new_idx = 0
        for ans in original_poll.answers:
            if ans.text is None:
                logging.warning("Poll answer text is None — skipping")
                continue
            new_option = bytes([new_idx])
            option_map[ans.option] = new_option
            clean_twe = remove_emojis_preserve_entities(ans.text)
            answers.append(types.PollAnswer(text=clean_twe, option=new_option))
            new_idx += 1

        # Remap the correct answer to our new byte scheme
        correct_option_mapped = option_map.get(correct_option)
        if correct_option_mapped is None:
            logging.error("Could not remap correct option — aborting poll send")
            if local_target:
                await client.send_message(local_target, "Error: could not determine correct answer")
            return

        # Newer Telethon requires hash= in Poll.__init__(); use 0 for new polls.
        poll = types.Poll(
            id=int(time.time()),
            hash=0,
            question=question,
        ...
        media = types.InputMediaPoll(
            poll=poll,
            correct_answers=[correct_option_mapped]
        )
